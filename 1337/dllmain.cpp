...
HRESULT present_hooked(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags) {
	...
	gui.begin_scene();
	{
		...
		if (no_menu::begin_window(_("1337 | android#1337"), no_menu::vec2({ 545, 580 }), VK_INSERT, no_menu::no_menu_window_flags_none))
		{
			...
			if (tab == 1) {
				...
				no_menu::slider_int(_("Aimbot Fov"), 10.f, 800.f, Settings::AimbotFovCircle);
			...
			if (tab == 3) {
				...
				no_menu::separator(10, false);
				no_menu::checkbox(_("Testtest"), Settings::DebugForTest);
			}
			...
			if (tab == 4) {
				...
				if (no_menu::button(_("Webiste"), no_menu::vec2({ 113, 32 }))) {
					system(_("start https://chea.to"));
				}
				no_menu::separator(10, false);
				if (no_menu::button(_("Discord Server"), no_menu::vec2({ 113, 32 }))) {
					system(_("start https://discord.chea.to"));
				}
			}
	...
	if (pFontWrapper)
	{
		pFontWrapper->DrawString(pContext, _(L"1337 | android#1337"), 14, 16.0f, 16.0f, 0xff0000ff, FW1_RESTORESTATE);
	}
...
void Init()
{
	...
	if (!No_Discord::module) {
		MessageBoxA(NULL, "Please enable discord overlay", "Error", MB_ICONERROR);
		exit(0);
	}
